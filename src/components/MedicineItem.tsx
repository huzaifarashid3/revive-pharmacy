...
export default function MedicineItem({ medicine, isAdmin, onEdit, onDelete, onMedicineClick }: MedicineItemProps) {
  ...
  return (
    <div 
      className={`bg-gray-800 border-l-4 ${stockStatus.border} rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:scale-[1.02] border border-gray-700 relative ${
        hasFormula 
    ...
    >
      <div className="p-3 sm:p-6">
        ...
        <div className="block sm:hidden">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center flex-1 min-w-0">
              ...
              <div className="flex-1 min-w-0">
                <h3 className="text-lg font-bold text-white truncate">{medicine.name}</h3>
                {hasFormula && (
                  <p className="text-xs text-green-400 font-medium truncate">{medicine.formula}</p>
                )}
              </div>
            </div>
            <div className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-bold ${stockStatus.bg} ${stockStatus.color} flex-shrink-0 ml-2`}>
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            <div className="flex items-center text-gray-300">
              ...
              <span className="truncate">{medicine.dosage || 'No dosage'}</span>
            </div>
            <div className="flex items-center text-gray-300">
              <svg className="w-3 h-3 text-green-400 mr-1 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 008 10.172V5L8 4z" />
              </svg>
              <span className="truncate">{medicine.formulation || 'No type'}</span>
            </div>
          </div>
        </div>
        ...
        {/* Admin buttons - positioned for both mobile and desktop */}
        {isAdmin && (
          <div className="flex gap-2 mt-3 sm:absolute sm:top-4 sm:right-4 sm:mt-0">
            <button
